...
ServerEvents.recipes((event) => {
	...
	let compactRecipe = (small, item1, tag1, item2, tag2) => {
		...
		if (small) {
			event.shaped(item1, ["ii", "ii"], { i: tag2 }).id(`${global.EE_PACKID}:${resultId}_from_${inputId}`);
			event.shapeless(`4x ${item2}`, tag1).id(`${global.EE_PACKID}:${inputId}_from_${resultId}`);
		} else {
			event.shaped(item1, ["iii", "iii", "iii"], { i: tag2 }).id(`${global.EE_PACKID}:${resultId}_from_${inputId}`);
			event.shapeless(`9x ${item2}`, tag1).id(`${global.EE_PACKID}:${inputId}_from_${resultId}`);
		}
	};
	...
	let { avaritia, create, bloodmagic, immersiveengineering, thermal, minecraft, mekanism } = event.recipes;
	...
	global.EE_MATERIALS.forEach(
		...
		(mat) => {
			...
			if (isIngredientExist(`#forge:ingots/${name}`)) {
				if (isIngredientExist(`#forge:nuggets/${name}`) && loadedMods.thermalfoundation) {
					if (recipesSwich.thermal && blacklist.thermal.press.includes(name) == false) {
						thermal.press(processedItems.ingot, [`9x #forge:nuggets/${name}`, "thermal:press_packing_3x3_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/packing3x3/${name}_ingot_3x3_packing`);
						thermal.press(`9x ${processedItems.nugget}`, [`#forge:ingots/${name}`, "thermal:press_packing_3x3_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/unpacking/${name}_ingot_unpacking`);
					}
				}
				...
				if (types.includes("storage_block")) {
					...
					if (loadedMods.thermalfoundation && recipesSwich.thermal && blacklist.thermal.press.includes(name) == false) {
						thermal.press(`${global.EE_PACKID}:${name}_block`, [`9x #forge:ingots/${name}`, "thermal:press_packing_3x3_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/packing3x3/${name}_block_3x3_packing`);
					}
				}
				if (types.includes("ore")) {
					minecraft.smelting(processedItems.ingot, `#forge:ores/${name}`).xp(0.7).id(`${global.EE_PACKID}:minecraft/smelting/${name}_ingot_from_ore`);
					minecraft.blasting(processedItems.ingot, `#forge:ores/${name}`).xp(0.7).id(`${global.EE_PACKID}:minecraft/blasting/${name}_ingot_from_ore`);
					if (loadedMods.thermalfoundation) {
						if (recipesSwich.thermal && blacklist.thermal.press.includes(name) == false) {
							thermal.smelter(processedItems.ingot, `#forge:ores/${name}`, 0.5, 3200).id(`${global.EE_PACKID}:thermal/smelter/smelter_${name}_ore`);
						}
					}
				}
				if (types.includes("raw")) {
					minecraft.smelting(processedItems.ingot, `#forge:raw_materials/${name}`).xp(0.7).id(`${global.EE_PACKID}:minecraft/smelting/${name}_ingot_from_raw`);
					minecraft.blasting(processedItems.ingot, `#forge:raw_materials/${name}`).xp(0.7).id(`${global.EE_PACKID}:minecraft/blasting/${name}_ingot_from_raw`);
					if (loadedMods.thermalfoundation) {
						if (blacklist.thermal.press.includes(name) == false) {
							thermal.smelter(processedItems.ingot, `#forge:raw_materials/${name}`, 0.5, 3200).id(`${global.EE_PACKID}:thermal/smelter/smelter_${name}_raw`);
						}
					}
				}
				if (isIngredientExist(`#forge:dusts/${name}`)) {
					minecraft.smelting(processedItems.ingot, `#forge:dusts/${name}`).xp(0.7).id(`${global.EE_PACKID}:minecraft/smelting/${name}_ingot_from_dust`);
					minecraft.blasting(processedItems.ingot, `#forge:dusts/${name}`).xp(0.7).id(`${global.EE_PACKID}:minecraft/blasting/${name}_ingot_from_dust`);
				}
				if (loadedMods.thermalfoundation && recipesSwich.thermal && blacklist.thermal.press.includes(name) == false) {
					thermal.smelter(processedItems.ingot, `#forge:dusts/${name}`, 0, 1600).id(`${global.EE_PACKID}:thermal/smelter/smelter_${name}_dust`);
				}
			}
			if (checkedTypes.raw) {
				if (checkedTypes.rawBlock) {
					...
					if (loadedMods.thermalfoundation && recipesSwich.thermal && blacklist.thermal.press.includes(name) == false) {
						thermal.press(processedItems.rawBlock, [`9x #forge:raw_materials/${name}`, "thermal:press_packing_3x3_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/packing3x3/raw_${name}_block_3x3_packing`);
						thermal.press(processedItems.raw, [`#forge:storage_blocks/raw_${name}`, "thermal:press_unpacking_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/unpacking/raw_${name}_block_unpacking`);
					}
				}
			...
			if (types.includes("gem") && types.includes("storage_block")) {
				...
				if (loadedMods.thermalfoundation && recipesSwich.thermal && blacklist.thermal.press.includes(name) == false) {
					thermal.press(`${global.EE_PACKID}:${name}_block`, [`9x #forge:gems/${name}`, "thermal:press_packing_3x3_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/packing3x3/${name}_block_3x3_packing`);
					thermal.press(`9x ${global.EE_PACKID}:${name}_gem`, [`#forge:storage_blocks/${name}`, "thermal:press_unpacking_die"], 0, 400).id(`${global.EE_PACKID}:thermal/press/unpacking/${name}_block_unpacking`);
				}
			}
			if (checkedTypes.gem) {
				...
				if (recipesSwich.immersiveengineering && checkedTypes.ore && !blacklist.immersiveengineering.crusher.includes(name) && loadedMods.immersiveengineering) {
					immersiveengineering.crusher(`2x #forge:gems/${name}`, `#forge:ores/${name}`).id(`${global.EE_PACKID}:immersiveengineering/crusher/${name}_ore`);
				}
			}
			if (checkedTypes.gear) {
				if (types.includes("ingot") || mat.type == "alloy" || mat.type == "metal" || mat.baseItem == "ingot")
					event
						.shaped(processedItems.gear, [" i ", "ini", " i "], {
							i: `#forge:ingots/${name}`,
				...
				if (recipesSwich.immersiveengineering && loadedMods.immersiveengineering && !blacklist.immersiveengineering.metalPress.includes(name))
					immersiveengineering.metal_press(`#forge:gears/${name}`, `4x #forge:${mat.baseItem}s/${name}`, ImmersiveEngineering.MOLDS.GEAR).id(`${global.EE_PACKID}:immersiveengineering/metal_press/${name}_gear`);
			}
			if (checkedTypes.rod) {
				if (checkedTypes.ingot) {
					event
						.shaped(`2x ${processedItems.rod}`, ["i", "i"], {
							i: `#forge:ingots/${name}`,
						})
				...
				if (recipesSwich.immersiveengineering && loadedMods.immersiveengineering && !blacklist.immersiveengineering.metalPress.includes(name))
					immersiveengineering.metal_press(`2x #forge:rods/${name}`, `#forge:${mat.baseItem}s/${name}`, ImmersiveEngineering.MOLDS.ROD).id(`${global.EE_PACKID}:immersiveengineering/metal_press/${name}_rod`);
			}
			if (checkedTypes.plate) {
				if (recipesSwich.create && loadedMods.create) {
					create.pressing(processedItems.plate, `#forge:${mat.baseItem}s/${name}`).id(`${global.EE_PACKID}:create/pressing/${name}_plate`);
				}
				if (recipesSwich.immersiveengineering && loadedMods.immersiveengineering) {
					if (blacklist.immersiveengineering.metalPress.includes(name) == false) {
						immersiveengineering.metal_press(`#forge:plates/${name}`, `#forge:${mat.baseItem}s/${name}`, ImmersiveEngineering.MOLDS.PLATE).id(`${global.EE_PACKID}:immersiveengineering/metal_press/${name}_plate`);
					}
					if (blacklist.immersiveengineering.hammerCraft.includes(name) == false) {
						minecraft.crafting_shapeless(processedItems.plate, [`#forge:${mat.baseItem}s/${name}`, "#immersiveengineering:tools/hammers"]).id(`${global.EE_PACKID}:minecraft/crafting/${name}_plate`);
					}
				}
				if (recipesSwich.thermal && loadedMods.thermalfoundation) {
					if (blacklist.thermal.press.includes(name) == false) {
						thermal.smelter(processedItems.ingot, processedItems.plate, 0, 1600).id(`${global.EE_PACKID}:thermal/smelter/smelter_${name}_plate`);
					}
				}
			}
			if (types.includes("mekanism") && recipesSwich.mekanism) {
				...
				mekanism
					.washing({ fluid: "minecraft:water", amount: 5 }, { amount: 1, slurry: `${global.EE_PACKID}:dirty_${name}` }, { amount: 1, slurry: `${global.EE_PACKID}:clean_${name}` })
					.id(`${global.EE_PACKID}:mekanism/processing/clean_${name}_slurry`);
				mekanism.crystallizing("slurry", `${global.EE_PACKID}:${name}_crystal`, { amount: 200, slurry: `${global.EE_PACKID}:clean_${name}` }).id(`${global.EE_PACKID}:mekanism/processing/${name}_crystal_from_slurry`);
				if (isIngredientExist(`#forge:raw_materials/${name}`)) {
					mekanism // raw to shard
					...
					mekanism
						.dissolution({ amount: 6000, slurry: `${global.EE_PACKID}:dirty_${name}` }, { amount: 2, gas: "mekanism:sulfuric_acid" }, `#forge:storage_blocks/raw_${name}`)
						.id(`${global.EE_PACKID}:mekanism/processing/dirty_${name}_slurry_from_raw_ore_block`);
					mekanism
						.dissolution({ amount: 2000, slurry: `${global.EE_PACKID}:dirty_${name}` }, { amount: 1, gas: "mekanism:sulfuric_acid" }, `3x #forge:raw_materials/${name}`)
						.id(`${global.EE_PACKID}:mekanism/processing/dirty_${name}_slurry_from_raw_ore`);
				}
				if (isIngredientExist(`#forge:ores/${name}`)) {
					...
					mekanism
						.dissolution({ amount: 1000, slurry: `${global.EE_PACKID}:dirty_${name}` }, { amount: 1, gas: "mekanism:sulfuric_acid" }, `#forge:ores/${name}`)
						.id(`${global.EE_PACKID}:mekanism/processing/dirty_${name}_slurry_from_ore`);
				}
			}
			...
			if (isIngredientExist(`#create:crushed_raw_materials/${name}`) && recipesSwich.create) {
				...
				if (checkedTypes.ingot) {
					...
					if (checkedTypes.raw) {
						create.crushing([processedItems.crushed, "create:experience_nugget 0.75"], `#forge:raw_materials/${name}`).id(`${global.EE_PACKID}:create/crushing/${name}_raw_ore`);
						create.crushing([`9 ${processedItems.crushed}`, "9 create:experience_nugget 0.75"], `#forge:storage_blocks/raw_${name}`).id(`${global.EE_PACKID}:create/crushing/${name}_raw_ore_block`);
					}
					minecraft.smelting(processedItems.ingot, processedItems.crushed).xp(0.1).id(`${global.EE_PACKID}:minecraft/smelting/${name}_ingot_from_crushed`);
					minecraft.blasting(processedItems.ingot, processedItems.crushed).xp(0.1).id(`${global.EE_PACKID}:minecraft/blasting/${name}_ingot_from_crushed`);
					create.splashing([parseChanceItem(`9 ${processedItems.nugget}`)], [{ tag: `create:crushed_raw_materials/${name}` }]).id(`${global.EE_PACKID}:create/splashing/crushed_${name}`);
				}
			}
			if (types.includes("thermal") && recipesSwich.thermal) {
				if (blacklist.thermal.press.includes(name)) return;
				if (loadedMods.thermalfoundation) {
					thermal.press(`3x ${global.EE_PACKID}:${name}_coin`, [`#forge:ingots/${name}`, "thermal:press_coin_die"]).id(`${global.EE_PACKID}:thermal/press/${name}_coin_from_ingot`);
					thermal.press(`${global.EE_PACKID}:${name}_coin`, [`3x #forge:nuggets/${name}`, "thermal:press_coin_die"], 0, 800).id(`${global.EE_PACKID}:thermal/press/${name}_coin_from_nuggets`);
				}
			}
